...
@Component({
  ...
  templateUrl: './reporthistory.page.html',
  styleUrls: ['./reporthistory.page.scss'],
})
...
esti_type:any
loading:any
bill_type:any ={'approval_type':'','id_branch':'','report_type':'','from_date':'','to_date':''}
empdata:any
receivedData:any
      ...
      if(data != null){
        ...
        data.forEach((element:any) => {
          ...
          if(element['bt_details'] != ''){
            ...
            /* history */
            element['bill_discount'] = formatter.format(element['bill_discount'])
            element['approved_discount'] = formatter.format(element['approved_discount'])
    ...
    let data1=
      {
        ...
        "apprl_approved_by" : this.empdata['emp_code'],
...
console.log("End date of the current week (Sunday):", formattedSundayDate);
   ...
   formatDate(date:any) {
    ...
    onDateChange_from(){
      this.closePopover()
      console.log(this.status_type,'type')
      this.responsedata = this.responsedata1.filter((item:any) => item['apprl_status'].toUpperCase().includes(this.status_type));
      console.log(this.responsedata);
     if(this.status_type == undefined){
